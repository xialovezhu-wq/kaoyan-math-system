...
def _display_assets_module(repo: Path) -> Any:
    path = repo / "数学一回滚复习系统/scripts/display_assets.py"
    if not path.is_file():
        raise ValueError("display_assets_module_missing")
    name = "math_formal_lint_display_assets_" + hashlib.sha256(
        str(path.resolve()).encode("utf-8")
    ).hexdigest()[:16]
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise ValueError("display_assets_module_invalid")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def _legacy_archive_module(repo: Path) -> Any:
    path = repo / "数学一回滚复习系统/scripts/archive_legacy_evidence.py"
    if not path.is_file():
        raise ValueError("legacy_archive_module_missing")
    name = "math_formal_lint_legacy_archive_" + hashlib.sha256(
        str(path.resolve()).encode("utf-8")
    ).hexdigest()[:16]
    spec = importlib.util.spec_from_file_location(name, path)
    if spec is None or spec.loader is None:
        raise ValueError("legacy_archive_module_invalid")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def _verified_archived_legacy_sources(
    repo: Path,
    formal_id: str,
    capture_refs: set[str],
    source_refs: set[str],
) -> dict[str, dict[str, str]]:
    """Resolve source tokens through strictly verified legacy archives.

    Historical source artifacts are deliberately removed from the live source
    staging tree after their immutable T9 archive, receipt, locator, and local
    pointer have all been verified. This helper accepts such a source token
    only when the card's exact capture token resolves to the same formal ID and
    the predecessor archive surfaces still reproduce the same source manifest.
    """
    if not FORMAL_ID_RE.fullmatch(formal_id) or not capture_refs or not source_refs:
        return {}
    try:
        canonical = _canonical_quick_intake(repo)
        events = canonical.load_jsonl(repo / LEDGER_RELATIVE_PATH)
        state = canonical.replay(events)
        legacy = _legacy_archive_module(repo)
    except (OSError, UnicodeError, ValueError, json.JSONDecodeError):
        return {}

    verified: dict[str, dict[str, str]] = {}
    for capture_ref in sorted(capture_refs):
        parts = capture_ref.split(":")
        if len(parts) != 4:
            continue
        capture_id = parts[1]
        ...
        if isinstance(capture.get("conversation_package"), dict):
            continue
        try:
            plan = legacy.build_legacy_archive_plan(
                repo=repo,
                capture_id=capture_id,
            )
            public = plan["public"]
            freeze = state.get("freezes", {}).get(public.get("freeze_id"))
            snapshots = freeze.get("capture_snapshots", []) if isinstance(freeze, dict) else []
            snapshot = next(
                (
                    row
            ...
            if not isinstance(snapshot, dict) or canonical.capture_ref_token(snapshot) != capture_ref:
                continue
            manifest_relative = public.get("source_bundle_manifest_path")
            manifest_hash = public.get("source_bundle_manifest_sha256")
            if (
                public.get("evidence_mode") != "source_bundle"
                or formal_id not in public.get("formal_ids", [])
            ...
            if not isinstance(locator, str) or not locator.strip():
                continue
            source_ref = _source_ref_token(locator, manifest_hash)
            if source_ref not in source_refs:
                continue
            legacy.verify_predecessor_archive_surfaces(plan)
        except (
            OSError,
            UnicodeError,
            ValueError,
...
def lint_card_structure(
    ...
    for value in lecture_refs:
        ...
        if suffix in IMAGE_SUFFIXES and (
            ...
            or normalized.startswith(("/Volumes/", "/private/", "/var/", "/tmp/"))
        ):
            issues.append(
                _structural_issue(
                    text,
                    "temporary_binary_reference_forbidden",
                    value,
                    "正式卡不得引用快速包、attachments、T9 或临时目录中的二进制；先提升到稳定 Obsidian 资产",
                )
            )
        if suffix in IMAGE_SUFFIXES and normalized.startswith("错题知识网络/"):
            asset = repo / value
            try:
                ...
                issues.append(
                    _structural_issue(
                        text,
                        "stable_visual_asset_missing",
                        value,
                        "正式卡中的稳定视觉资产不存在或不可读取",
                    )
                )
    manifests = [value for value in lecture_refs if value.endswith("manifest.json")]
    archived_legacy_sources = _verified_archived_legacy_sources(
        repo,
        formal_id,
        capture_refs,
        source_refs,
    )
    expected_source_refs: set[str] = set(archived_legacy_sources)
    for relative in manifests:
        ...
        manifest_source_ref = _source_ref_token(locator, manifest_hash)
        expected_source_refs.add(manifest_source_ref)
        archived_binding = archived_legacy_sources.get(manifest_source_ref)
        archived_manifest_verified = bool(
            archived_binding
            and archived_binding.get("manifest_path") == relative
            and archived_binding.get("manifest_sha256") == manifest_hash
        )
        display_roles = {
            "question_image",
            "question",
        ...
        needs_display_closure = any(
            isinstance(artifact, dict)
            and artifact.get("role") in display_roles
            and Path(str(artifact.get("path", ""))).suffix.lower() in IMAGE_SUFFIXES
            for artifact in artifacts
        )
        display_verified = False
        display_error = ""
        if needs_display_closure and not archived_manifest_verified:
            try:
                display = _display_assets_module(repo)
                display.verify_closure_for_formal(
                    repo=repo,
                ...
                try:
                    display.verify_historical_closure_for_formal(
                        repo=repo,
                        formal_id=formal_id,
                    )
                    display_verified = True
                except (OSError, UnicodeError, ValueError) as historical_exc:
                    display_error = (
                        f"canonical={canonical_error};"
                        f"historical={historical_exc}"
                    )
                    issues.append(
                        _structural_issue(
                            text,
                            "display_asset_closure_missing_or_invalid",
                            relative,
                            f"带图片的正式来源缺少可重验的稳定展示闭环：{display_error}",
                        )
                    )
        for artifact in artifacts:
            ...
            try:
                ...
                actual_hash = None
            artifact_missing = actual_hash is None
            if (
                actual_hash != expected_hash
                and not display_verified
                and not (artifact_missing and archived_manifest_verified)
            ):
                issues.append(
                    _structural_issue(
                        text,
    ...
    if source_refs and not manifests and not source_refs.issubset(expected_source_refs):
        issues.append(
            _structural_issue(
                text,
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument(
        "--repo",
        required=True,
        help="数学仓库根目录；必须显式传入，不读取中央 Study Intake config",
    )
...
def main() -> int:
    ...
    try:
        ...
        card_root = (repo / "错题知识网络" / "错题卡").resolve(strict=True)
